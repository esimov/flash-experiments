package 
{
	 ...
	 * @author Simo Endre
	 */
	public class FrozenTree extends Sprite
	{
		...
		private var elem:Vector.<Number> = new Vector.<Number>();
		
		public function FrozenTree():void 
		{
			if (stage) init();
			else {
		...
		private function createTrees():void
		{
			...
			trees = new Vector.<Tree>(numTrees, false);
			for (i = 0; i < numTrees; i++) 
			{
				trees.push(new Tree( -1, 3 * Math.PI / 2, totalBranchLength, 10));
			}
		}
		...
		private function drawTrees():void
		{
			...
			{
				elem.push((1+i)*(STAGE_WIDTH/(1+numTrees)), STAGE_HEIGHT);
				Tree(trees[i]).drawBranches(elem);
			}
		}
