package  
{
	...
	import com.esimov.utils.LinkedList;
	import com.esimov.utils.LinkedListNode;

	[SWF(backgroundColor = '0x00', width = '400', height = '400', frameRate=60)]

	/**
	 * @author simoe
	 */
	public class TuringPatternLL extends Sprite
	{
		...
		public function TuringPatternLL() : void
		{
			if (stage)
			{
				initStage(null);
		...
		private function initStage(event : Event) : void
		{
			...
			var base : Number = random(1.2, 2.5);
			var stepScale : Number = random(.005, .06);
			var stepOffset : Number = random(.0001, .012);
		...
		final private function step() : void
		{
		...
		final private function click(event : MouseEvent) : void
		{
			var i : int;
			for (i = 0;i < n;i++)
			{
		...
		final private function update(event : Event) : void
		{
			...
			step();
			_bmpData.lock();
			for (curNode = _list.head; curNode; curNode =  curNode.next)
			{
				var p : * = curNode.data;
		...
		final private function getPixels() : void
		{
			var x : Number;
			var y : Number;
		...
		final private function fastBlur(source : Vector.<Number>, dest : Vector.<Number>, buffer : Vector.<Number>, 
