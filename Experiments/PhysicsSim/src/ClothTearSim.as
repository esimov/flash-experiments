package
{
	...
	public class Main extends Sprite
	{
		...
		private function onClothPress(event:MouseEvent):void
		{
			if (event.ctrlKey)
			{
				ps.removeParticle(getParticleIndex());
				FlashConnect.trace(getParticleIndex());
				count++;
			}
			mouseDown = true;
		...
		private function getParticleIndex():int
		{
			particles.results = [];
			particles.target = searchDraggingPoint();
			particles.forEach(searchIndex);
			
			return ppos;
		}
		
		private function searchIndex(element:*, index:int, bin:Vector.<Particle>):int
		{
			for (var i:* in bin)
			{
				if (element[i].indexOf(bin.target) > -1)
				{
					var tmp:Vector.<Particle> = bin.results;
					tmp.push([index, i]);
					bin.results = tmp;
					break;
				}
			}
			return ppos = bin.results[i];
		}
		...
		private function searchDraggingPoint():Particle
		{
			...
			for (var i:int = 0; i < numRows-count; i++)
			{
				for (var j:int = 0; j < numCols-count; j++)
				{
		...
		private function renderCloth(event:Event):void
		{
			...
			for (var i:int = 0; i < numRows-count; i++)
			{
				for (var j:int = 0; j < (numCols-1)-count; j++)
				{
			...
			for (i = 0; i < (numRows-1)-count; i++)
			{
				for (j = 0; j < numCols-count; j++)
				{
					canvas.graphics.moveTo(particles[i][j].position.x, particles[i][j].position.y);
