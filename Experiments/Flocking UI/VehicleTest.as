	...
	public class VehicleTest extends Sprite
	{
		...
		public function VehicleTest()
		{
			...
			for (var i:int = 0; i<= NUM_VEHICLES; i++)
			{
				...
				_vehicle.maxForce = Math.random() * 2 + 1;
				_vehicle.mass = 2 - Math.random() ;
				_vehicle.maxSpeed = Math.random() * 4 + 3;
				_vehicle.velocity.length = Math.random() * 2 + 2;
				_vehicle.edgeBehavior = "bounce";
				//_vehicle.velocity.angle = Math.PI / (4 - Math.random() * 2);
				addChild(_vehicle);
		...
		private function onEnterFrame(event:Event):void
		{
			...
			{
				var prev:SteeredVehicle = SteeredVehicle(_vehicles[i]);
				prev.arrive(new Vector2D(mouseX, mouseY));
				prev.wander();
				prev.update();
				var next:SteeredVehicle = SteeredVehicle(_vehicles[i+1]);
				next.pursue(prev);
				next.wander();
				next.update();
				prev.avoid(_obstacles);
			}
		}
	}
}