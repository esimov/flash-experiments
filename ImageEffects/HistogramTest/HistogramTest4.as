package {
	...
	public class HistogramTest4 extends AbstractImageLoader {
		...
		private var _bitmapVectorData:Vector.<BitmapData>;
		...
		public function HistogramTest4():void {
		...
		*/
		private function setLevels(bitmap:BitmapData, min:uint, mid:uint, max:uint):void {
			var tempBitmapData:BitmapData;
			_bitmapVectorData = new Vector.<BitmapData>();
			min = _level.lowValue;
			...
			tempBitmapData = bitmap.paletteMap(bitmap, bitmap.rect, new Point(), r, g, b) as BitmapData;
			_bitmapVectorData.push(tempBitmapData);
...
*/		}
		...
		private function levelSliderHandler(evt:Event):void {
			if (_level.lowValue < _max || _level.highValue > _max){
				for (var i:int = 0; i <= _bitmapVectorData.length; i++) {
					var bitmap:BitmapData = _bitmapVectorData[i] as BitmapData;
					setLevels(bitmap, _level.lowValue, _max, _level.highValue);
